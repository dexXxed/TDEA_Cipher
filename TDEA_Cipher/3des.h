...
public:
    ...
    inline std::string encrypt(const char* data, int len) {
        ...
        if (!iv_.empty()) {
            ...
            data_.crypt_cbc(&data_.ctx_, DES_ENCRYPT, new_len, iv, after_padding, (unsigned char*)ret.data());
        } else {
    ...
    inline std::string decrypt(const char* data, int len, bool padding = true) {
        ...
        if (!iv_.empty()) {
            unsigned char iv[8] = { 0 };
            memcpy(iv, (unsigned char*)iv_.data(), std::min(8u, iv_.size()));
            data_.crypt_cbc(&data_.ctx_, DES_DECRYPT, new_len, iv, (const unsigned char*)data, (unsigned char*)ret.data());
        } else {
            unsigned char* output = (unsigned char*)ret.data();
...
struct DES3_Trait {
    static constexpr auto setkey_enc = des3_set3key_enc;
    static constexpr auto setkey_dec = des3_set3key_dec;
    static constexpr auto crypt_cbc = des3_crypt_cbc;
    static constexpr auto crypt_ecb = des3_crypt_ecb;
